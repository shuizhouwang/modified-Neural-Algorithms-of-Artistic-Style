...
def resnet(inpt, n):
    ...
    for i in range (num_conv):
        ...
        with tf.variable_scope('conv3_%d' % (i+1)):
            conv3_x,conv3_x_w = residual_block(out_list[-1], 32, down_sample)
            conv3,conv3_w = residual_block(conv3_x, 32, False)
            layers['conv3_x_'+ str(i)] = conv3_x
            layers['conv3_'+ str(i)] = conv3
            
            out_list.append(conv3_x)
            out_list.append(conv3)
    ...
    for i in range (num_conv):
        ...
        with tf.variable_scope('conv4_%d' % (i+1)):
            conv4_x,conv4_x_w = residual_block(out_list[-1], 64, down_sample)
            conv4,conv4_w = residual_block(conv4_x, 64, False)
            layers['conv4_x_'+ str(i)] = conv4_x
            layers['conv4_'+ str(i)] = conv4
            
            out_list.append(conv4_x)
            out_list.append(conv4)
